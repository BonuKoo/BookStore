...
> **Toss Payments 연계 전자상거래 흐름**을 구현하고, 그 위에 **결제 확정 이후의 미완 흐름**
> (재고 차감·판매자 정산·복식부기 장부·알림)을 **물리적으로 분리된 3대 PC의 RabbitMQ 파이프라인**으로 완성한 프로젝트.

단순 기능 구현을 넘어, 분산 시스템의 **신뢰성(유실·중복 없음)을 코드로 보장**하고
장애 주입·부하 테스트로 **실제 병목과 동시성 버그를 발견·진단**하는 데 초점을 맞췄습니다.

...
- 2026-07-15(후속): Phase 4 실브로커 e2e 잔여 항목 전부 완료 — 강제 예외→DLQ 적재/x-death 헤더/멱등성 확인 + PC3 워커 3종 신코드(manual ack+DLX) 배포 및 `ledger.recording.queue.dlq`에서 워커측 DLQ 동작 실증. PC2 절전 문제도 전원옵션 조치로 해결. **Phase 4 완전 종료.**
- 2026-07-15(후속): Phase 5 착수. 실험 A(발행 중 PC2 차단) 완료 — PC1 방화벽으로 PC2:5672 아웃바운드 차단 → outbox 릴레이가 예외 catch 후 `FAILURE` 마킹(유실 없음) → 차단 해제 후 다음 릴레이 주기에 자동 `SUCCESS` 전환 확인, 사람 개입 불필요. 상세 결과는 `docs/failure-experiments.md`. 실험 B/C는 PC3 물리 접근이 필요해 사용자 수동 진행으로 대기 중.
- 2026-07-15(후속): 실험 B 완료(ledger-worker, 5건 적재→재기동 시 자동 캐치업, 부팅~7-10초 실측) — durable queue 설계 실증.
- 2026-07-15(후속): 실험 C(ack 전 강제종료→재전달)는 3회 시도 모두 재전달 미재현으로 보류. 원인: RabbitMQ 관리 API 통계 집계 지연(5초)과 PC3 잔여 좀비 프로세스가 결과를 왜곡시킴, `Stop-Process -Force`도 간헐적으로 무효화됨(원인 미상). 상세 원인 분석·향후 재시도 방법은 `docs/failure-experiments.md` 참고. **Phase 5 사실상 종료** — 남은 건 우선도 낮은 Phase 6(선택)뿐.
- 잔여: Phase 6(선택, Grafana+k6) 외 없음.
